"""
Data loading and validation module for spam detection.
...
from .utils import DATA_DIR, setup_logger, LABEL_MAP
...
class DataLoader:
    ...
    def load_sms_spam_collection(
        ...
        """
        Load the SMS Spam Collection Dataset directly from URL or local file.
    ...
    def _load_local_fallback(self) -> pd.DataFrame:
        ...
        Returns:
            DataFrame with SMS data or raises FileNotFoundError
        """
        local_paths = [
    ...
    def get_train_test_split(
        ...
        Args:
            data: DataFrame to split (uses internal _data if None)
    ...
    def load_messages_for_inference(
        self, messages: Union[str, List[str]]
    ) -> pd.Series:
        """
        Load messages for inference (single prediction or batch).
